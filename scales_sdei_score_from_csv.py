...
EVIDENCE_CSV = Path("ops/SDEI_Evidence.csv")
OUTPUT_STATE = Path("data/modules/SDEI_state.json")
...
def load_evidence():
    ...
    if missing:
        raise ValueError(f"SDEI evidence CSV missing columns: {missing}")
    return df
...
def main():
    ...
    sdei_conf = modules_conf.get("SDEI", {})
    comps_conf = sdei_conf.get("components", [])
    scoring_conf = sdei_conf.get("scoring", {})
    ...
    state = {
        ...
        "as_of": "2025-12-06",
        "key": "SDEI",
        "name": "Surveillance & Data Exploitation Index",
        "score": round(score_norm, 1) if score_norm is not None else None,
        "band": module_band,
        "summary": "Illustrative SDEI score computed from component-level evidence.",
        "components": components_out,
        "notes": [
            "Scores in this alpha run are illustrative and based on a small evidence set.",
            "Production runs should use a richer, regularly updated SDEI_Evidence.csv."
        ]
    }
    ...
    print(f"Wrote SDEI state to {OUTPUT_STATE}")
